...
class BottomNavigationJetpackVM(
    ...
    val uiStateFlow = MutableStateFlow(
        BottomNavigationJetpackUiState(
            currentTab = BottomNavigationItem.Setting,
    ...
    private fun onTabClick(newTab: BottomNavigationItem) {
        val current = uiStateFlow.value.currentTab
    ...
    ): Boolean = items.firstOrNull { it.first == this?.id }?.second == uiStateFlow.value.currentTab
