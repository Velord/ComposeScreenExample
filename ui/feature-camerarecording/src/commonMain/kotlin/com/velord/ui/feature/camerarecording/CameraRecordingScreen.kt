...
private fun Content(
    ...
    Box(
        ...
        Recording(
            uiState = uiState,
            onStartStopRecording = { onAction(CameraRecordingUiAction.StartStopRecording) },
        )
...
private fun BoxScope.Recording(
    ...
    if (permissionCamera.isGranted && permissionAudio.isGranted) {
        CameraRecordingPreview(session = uiState.cameraSession)
        Adjustments(
            uiState = uiState,
            onStartStopClick = onStartStopRecording,
        )
    }
}

@Composable
private fun CameraRecordingPreview(session: CameraSessionWrapper?) {
...
private fun BoxScope.CameraSelector(
    ...
    Column(
        ...
        CameraSelectorIcon(onClick = onChangeCameraSelector)
    }
}
