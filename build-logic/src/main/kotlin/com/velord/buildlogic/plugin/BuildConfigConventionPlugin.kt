package com.velord.buildlogic.plugin

import com.velord.buildlogic.util.BuildKonfigFlavorResolver
import com.velord.buildlogic.util.applyPlugin
import org.gradle.api.Plugin
import org.gradle.api.Project

private const val BUILD_KONFIG_FLAVOR_PROPERTY = "buildkonfig.flavor"

class BuildConfigConventionPlugin : Plugin<Project> {

    override fun apply(target: Project) {
        with(target) {
            applyPlugin("convention-kmp-library")
            configureBuildKonfigFlavor()
            applyPlugin("buildkonfig")
        }
    }

    // BuildKonfig generates one global object per invocation, not one per Android variant.
    // Full variant tasks can select one matching flavor. App aggregate tasks are rejected because
    // one generated object cannot represent multiple variants in the same invocation.
    private fun Project.configureBuildKonfigFlavor() {
        val flavor = BuildKonfigFlavorResolver.resolve(gradle.startParameter.taskNames)
        if (hasProperty(BUILD_KONFIG_FLAVOR_PROPERTY)) return

        flavor?.let {
            extensions.extraProperties.set(BUILD_KONFIG_FLAVOR_PROPERTY, flavor)
        }
    }
}
